...
function homePage() {
  const themeCards = themes.map((theme) => `<li><a class="theme-link" href="${url(`/research/#${theme.id}`)}"><span class="theme-number">0${theme.order}</span><span class="theme-frequency">${theme.order === 1 ? "DEVICE" : theme.order === 2 ? "MEASURE" : "DEPLOY"}</span><h3>${esc(theme.title)}</h3><p>${esc(theme.problem)}</p><span class="theme-methods">${esc(theme.methods.slice(0,2).join(" · "))}</span><span class="theme-arrow" aria-hidden="true">↗</span></a></li>`).join("");
  ...
  <section class="section section-paper featured-work" aria-labelledby="featured-work"><div class="wide-container"><div class="section-heading"><div>${eyebrow("Selected evidence")}<h2 id="featured-work">Work at the edge of measurement</h2></div><a class="section-link" href="${url("/publications/")}">All eight publications <span aria-hidden="true">↗</span></a></div><div class="featured-grid">${featuredPublications.map((pub) => publicationCard(pub, true)).join("")}</div></div></section>
  <section class="section current-work" aria-labelledby="current-work"><div class="wide-container"><div class="section-heading"><div>${eyebrow("September 2026 snapshot")}<h2 id="current-work">Systems in progress</h2></div><a class="section-link" href="${url("/research/#current-projects")}">All research projects <span aria-hidden="true">↗</span></a></div><div class="project-grid">${homeProjects.map(projectCard).join("")}</div></div></section>
  <section class="section split-band" aria-labelledby="teaching-leadership"><div class="wide-container"><h2 id="teaching-leadership" class="visually-hidden">Teaching and technical communities</h2><div><span class="section-index">Teaching</span><h3>RF engineering in the laboratory</h3><p>I teach laboratory courses in high-frequency technology, radiofrequency subsystems, and linear network analysis at UC3M.</p><a href="${url("/teaching/")}">Teaching & mentoring</a></div><div><span class="section-index">Technical communities</span><h3>Building systems and teams</h3><p>I hold founding leadership roles in PUNTO and UC3-Base and contribute to payload and radio-frequency work with ST3LLARSAT1/BOIRA.</p><a href="${url("/engagement/")}">Engagement</a></div></div></section>
  <section class="section bio-contact" aria-labelledby="about-juan"><div class="wide-container two-column"><div>${eyebrow(person.location)}<h2 id="about-juan">About me</h2><p>${esc(person.shortBio)}</p><a href="${url("/about/")}">Read my full biography</a></div><aside class="contact-panel"><h3>Research and collaboration</h3><p>For research, technical collaboration, or student-project questions, contact me through my UC3M address.</p><a class="button button-primary" href="mailto:${esc(person.email)}">Email me</a>${profileLinks()}</aside></div></section>`;
...
function researchPage() {
  ...
  return layout({title:"Research", description:"My research themes, methods, and selected projects in integrated SiGe systems, antennas, radiometry, sensing, and communications.", active:"research", route:"/research/", content});
}
...
function teachingPage() {
  const courses = teaching.courses.map((course) => `<article class="course"><div><span>${esc(course.level)}</span><h3>${esc(course.title)}</h3></div><dl><div><dt>Role</dt><dd>${esc(course.role)}</dd></div><div><dt>Academic years</dt><dd>${esc(course.academicYears.join(", "))}</dd></div></dl><ul class="tag-list" aria-label="Course topics">${course.topics.map((topic) => `<li><span>${esc(topic)}</span></li>`).join("")}</ul></article>`).join("");
  const sup = teaching.supervision[0];
  const content = `${pageIntro("Teaching & mentoring", "Engineering through practice", teaching.statement)}<section class="section section-paper" aria-labelledby="courses"><div class="wide-container"><div class="section-heading"><div>${eyebrow("UC3M")}<h2 id="courses">Courses</h2></div></div><div class="course-list">${courses}</div></div></section><section class="section" aria-labelledby="student-projects"><div class="wide-container two-column"><div><h2 id="student-projects">Student projects</h2><p>In my mentoring, I connect antenna, circuit, and measurement questions to a defined engineering problem.</p></div><article class="supervision"><span>${esc(sup.year)} · ${esc(sup.level)}</span><h3>${esc(sup.title)}</h3><p>${esc(sup.role)}</p><a href="${url("/research/#integrated-sige-front-ends")}">Explore related research</a></article></div></section><section class="section contact-strip"><div class="wide-container"><div><h2>Ask about a student project</h2><p>${esc(teaching.inquiry)}</p></div><a class="button button-primary" href="mailto:${esc(person.email)}">Email me</a></div></section>`;
  return layout({title:"Teaching & Mentoring", description:"My laboratory teaching and student mentoring in high-frequency technology, radiofrequency subsystems, and network analysis at UC3M.", active:"teaching", route:"/teaching/", content});
}
...
function engagementPage() {
  ...
  const content = `${pageIntro("Technical communities", "Engagement", "Through technical leadership, conference presentations, and hands-on student initiatives, I connect my research practice with wider engineering communities.")}<section class="section section-paper" aria-labelledby="communities"><div class="wide-container"><div class="section-heading"><div>${eyebrow("Leadership and service")}<h2 id="communities">Technical communities</h2></div></div><div class="organization-grid">${orgs}</div></div></section><section class="section" aria-labelledby="presentations"><div class="wide-container"><div class="section-heading"><div>${eyebrow("Selected record")}<h2 id="presentations">Talks & presentations</h2></div></div><ol class="timeline">${talkList}</ol></div></section><section class="section membership-band"><div class="wide-container two-column"><div><h2>Professional memberships</h2><p>I am a member of the Institute of Electrical and Electronics Engineers (IEEE) and the Official College of Telecommunications Engineers (COIT).</p></div><div><h2>Contact</h2><p>For technical-community and presentation enquiries, contact me at my institutional email address.</p><a href="mailto:${esc(person.email)}">${esc(person.email)}</a></div></div></section>`;
  return layout({title:"Engagement", description:"My technical-community leadership, selected talks, CubeSat and rocketry work, and professional engagement.", active:"engagement", route:"/engagement/", content});
}
